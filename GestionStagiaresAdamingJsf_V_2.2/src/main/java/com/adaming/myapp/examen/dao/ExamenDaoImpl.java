...
public class ExamenDaoImpl implements IExamenDao {
	...
	public Object getScoreExamen(Long idSession, Long idEtudiant, Long idModule) {
		Query query = em
				.createQuery("select sum(e.note)from Examen e where e.sessionEtudaint.idSession=:x and e.etudiant.idEtudiant=:y and e.module.idModule=:z ");
		query.setParameter("x", idSession);
		query.setParameter("y", idEtudiant);
		query.setParameter("z", idModule);
		log.info("la note de letudiant: " + idEtudiant + " pour le module "
				+ idModule + " est : " + query.getSingleResult());
