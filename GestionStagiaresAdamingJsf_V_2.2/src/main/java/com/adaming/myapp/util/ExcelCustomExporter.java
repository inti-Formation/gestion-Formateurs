...
public class ExcelCustomExporter extends Exporter {
	...
	public void export(ActionEvent event, String tableId, FacesContext context,
		...
		while (st.hasMoreElements()) {
			...
			UIComponent component = SearchExpressionFacade.resolveComponent(
					context, event.getComponent(), tableName);
			if (component == null) {
				throw new FacesException("Cannot find component \"" + tableName
						+ "\" in view.");
			}
			...
			if (preProcessor != null) {
				preProcessor
						.invoke(context.getELContext(), new Object[] { wb });
			}
			if (tableTitle != null && !tableTitle.isEmpty()
					&& !tableId.contains("" + ",")) {
				Row titleRow = sheet.createRow(sheet.getLastRowNum());
				int cellIndex = titleRow.getLastCellNum() == -1 ? 0 : titleRow
						.getLastCellNum();
			...
			sheet.createRow(sheet.getLastRowNum()
					+ Integer.parseInt(datasetPadding));
		}
	...
	protected void exportAll(FacesContext context, DataTable table,
		...
		if (subTable) {
			...
			if (table.getHeader() != null) {
				tableFacet(context, sheet, table, subTableColumnsCount,
						"header");
			}
			...
			while (subTableCount > 0) {
				...
				if (subtable.getHeader() != null) {
					tableFacet(context, sheet, subtable, subTableColumnsCount,
							"header");
				}
	...
	protected void exportPageOnly(FacesContext context, DataList list,
			Sheet sheet) {
		int first = list.getFirst();
	...
	protected void tableFacet(FacesContext context, Sheet sheet,
		...
		if (component != null) {
			...
			sheet.addMergedRegion(new CellRangeAddress(sheetRowIndex, // first
																		// row
																		// (0-based)
	...
	protected void tableColumnGroup(Sheet sheet, DataTable table,
			String facetType) {
	...
	protected void tableColumnGroup(Sheet sheet, SubTable table,
			String facetType) {
		ColumnGroup cg = table.getColumnGroup(facetType);
		List<UIComponent> headerComponentList = null;
		if (cg != null) {
				...
				if (rowExpansion.getChildren() != null) {
					...
					if (rowExpansion.getChildren().get(0) instanceof DataTable) {
						...
						if (childTable.getHeader() != null) {
							tableFacet(context, sheet, childTable,
									columnsCount, "header");
						...
						if (childTable.hasFooterColumn()) {
							addColumnFacets(childTable, sheet,
									ColumnType.FOOTER);
						}
		...
		for (UIComponent component : list.getChildren()) {
			if (component instanceof Column) {
				...
				for (UIComponent childComponent : column.getChildren()) {
					int cellIndex = row.getLastCellNum() == -1 ? 0 : row
							.getLastCellNum();
	...
	protected void addColumnFacets(SubTable table, Sheet sheet,
			ColumnType columnType) {
	...
	protected CellStyle addColumnAlignments(UIComponent component,
			CellStyle style) {
	...
	protected void createCustomFonts() {
		...
		if (facetBackground != null) {
			...
			((XSSFCellStyle) facetStyle)
					.setFillForegroundColor(backgroundColor);
	...
	protected void writeExcelToResponse(ExternalContext externalContext,
			org.apache.poi.ss.usermodel.Workbook generatedExcel, String filename)
			throws IOException {
