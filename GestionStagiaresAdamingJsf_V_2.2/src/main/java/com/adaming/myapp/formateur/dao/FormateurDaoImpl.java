...
public class FormateurDaoImpl implements IFormateurDao {
			...
			if(formateur != null){
				if(formateur.getDateDeNaissance().compareTo(f.getDateDeNaissance()) ==0
						&& formateur.getNom().equals(f.getNom())){
							throw new VerificationInDataBaseException("Le Formateur "+f.getNom()+" Existe déja dans la base de données");
				}
			}
			
		}
		em.persist(f);
		logger.info("le formateur "+f.getNom()+"à bien été criée avec success");
		return f;
	}
	...
	public void addFormateurToSession(Long idSession, Long idFormateur) {
		...
		logger.info("le formateur "+f.getNom()+" a bien été attribué à la session "+session.getIdSession());
	}
